...
**Write-flag on a plain read-only entry, and TWO getopt differentials in the guard (claude-pilot#64).** `sort` sat on `SAFE_SHELL_COMMANDS` unconditionally because `sort <file>` is read-only — but `sort -o FILE` / `--output=FILE` writes its output to an arbitrary path. This is the §6(b) "entry's own flags write" class again (sibling to `find -fprintf`, `command tee`): a Tier-1-reachable arbitrary file-write, including the control plane (`sort --o=.git/hooks/post-checkout <content>` → code execution on next checkout), caught by neither the Tier-3 `>` pattern (it's a flag, not a redirect) nor the cpp#42 destination validator (which lives in Tier 2, never reached because Tier 1 already auto-approved). The fix mirrors `find`/`xargs`/`command`: `_is_safe_sort_command` special-cased in `is_safe_shell_command`, closed-world denying any output-flag shape and routing it to relay. But writing that guard surfaced **two new getopt-differential traps** (§6's xargs lesson, generalized) — both found by adversarial + correctness review (§3), neither obvious from the canonical `--output` spelling, both verified against the **real** GNU coreutils 9.9 binary:

- **(c) GNU getopt accepts any UNAMBIGUOUS PREFIX abbreviation of a long option.** An exact `--output` / `--output=` match is bypassable: `sort --out=FILE`, `--o=FILE`, `--outp FILE` all bind to `--output` and write (live-confirmed: each wrote the destination, exit 0). `--output` is `sort`'s only `--o…` option, so every prefix down to `--o` resolves to it. The sound guard denies any long token whose name (before `=`) is a non-empty prefix of `--output` (`len(name) >= 3 and "--output".startswith(name)`) — no read-only `sort` long option begins with `--o`, so this over-blocks nothing. **General rule: a security gate that string-matches a long flag must match the abbreviation class, not the canonical spelling.**
- **(d) A value-taking short flag's attached value is data, not flags.** The first short-flag cut denied on `"o" in cluster` — but `sort`'s value-taking short flags `-k`/`-S`/`-t`/`-T` carry an attached value that commonly contains the letter `o`: `sort -T/tmp/foo` (temp dir — paths routinely contain `o`), `sort -to` (separator literally `o`) were **false-denied** (routed to relay though read-only). The fix walks the cluster left-to-right with getopt semantics: `-o` is the write flag → deny; the other value-taking flags consume the rest of the token → stop scanning (so `-to`/`-T…o` are data, allowed) while `-uo`/`-oF` (the `-o` reached before any value-taking flag) still deny. This is the §6 xargs lesson (line ~286) from the *over-block* side: skipping or scanning a cluster by letter is parsing, and the arity of every short flag must match real `getopt` — a value-taking flag swallows what follows it.

The compounded §6 rule, now at n=5 (`find`/`rg`, `xargs`, `command`, `sort`): an allowlist entry is sound only if (i) the binary cannot exec/write through its own flags — audit `--output`/`--pre`/`-fprintf`/`tee`-class flags per entry; (ii) the guard's flag model matches the **real** getopt, on BOTH axes — long-option prefix abbreviation (don't under-match the deny) and value-taking-flag attached values (don't over-match into the value). Verify against the actual binary, never the man-page spelling.
